...
class MotionGroup_ScrollCaretIntoViewHorizontally_Test : VimTestCase() {
  ...
  fun `test moving right with sidescroll 1`() {
    VimPlugin.getOptionService().setOptionValue(OptionScope.GLOBAL, OptionConstants.sidescroll, VimInt(1))
    configureByColumns(200)
  ...
  fun `test moving right with sidescroll 2`() {
    VimPlugin.getOptionService().setOptionValue(OptionScope.GLOBAL, OptionConstants.sidescroll, VimInt(2))
    configureByColumns(200)
    typeText(injector.parser.parseKeys("80|" + "l"))
  ...
  fun `test moving right with large sidescrolloff keeps cursor centred`() {
    VimPlugin.getOptionService().setOptionValue(OptionScope.GLOBAL, OptionConstants.sidescrolloff, VimInt(999))
    configureByColumns(200)
