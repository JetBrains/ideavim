...
@ApiStatus.Internal
abstract class VimTestCaseBase : VimTestCase {
  ...
  override fun configureByText(fileType: com.maddyhome.idea.vim.model.FileType, content: String): VimEditor {
    val fType = when (fileType) {
      com.maddyhome.idea.vim.model.FileType.PLAIN_TEXT -> PlainTextFileType.INSTANCE
        com.maddyhome.idea.vim.model.FileType.XML -> XmlFileType.INSTANCE
        com.maddyhome.idea.vim.model.FileType.JSON -> JsonFileType.INSTANCE
    }
    return configureByText(fType, content)
  }
  ...
  override fun typeText(editor: VimEditor, keys: List<KeyStroke?>): VimEditor {
    ...
    when (Checks.keyHandler) {
      Checks.KeyHandlerMethod.DIRECT_TO_VIM -> typeText(keys.filterNotNull(), editor.ij, project)
      Checks.KeyHandlerMethod.VIA_IDE -> typeTextViaIde(keys.filterNotNull(), editor.ij)
    }
    return editor
  }
  ...
  override fun assertPosition(line: Int, column: Int) {
  ...
  override fun assertOffset(vararg expectedOffsets: Int) {
    val carets = fixture.editor.caretModel.allCarets
  ...
  override fun assertExOutput(expected: String) {
  ...
  override fun performTest(keys: String, after: String, modeAfter: Mode) {
    typeText(keys)
  ...
  override fun setRegister(register: Char, keys: String) {
