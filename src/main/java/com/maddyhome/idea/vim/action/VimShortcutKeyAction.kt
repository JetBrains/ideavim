...
internal class VimShortcutKeyAction : AnAction(), DumbAware/*, LightEditCompatible*/ {
  ...
  override fun actionPerformed(e: AnActionEvent) {
    ...
    val keyStroke = getKeyStroke(e)?.vim
    if (editor != null && keyStroke != null) {
      ...
      if ((owner as? ShortcutOwnerInfo.AllModes)?.owner == ShortcutOwner.UNDEFINED) {
        VimPlugin.getNotifications(editor.project).notifyAboutShortcutConflict(keyStroke.swing)
      }
  ...
  private fun isEnabled(e: AnActionEvent, keyStroke: KeyStroke?): ActionEnableStatus {
    ...
    val info = savedShortcutConflicts[keyStroke?.vim]
    return when (info?.forEditor(editor.vim)) {
      ...
      ShortcutOwner.IDE -> {
        ...
        if (isShortcutConflict(keyStroke)) {
          savedShortcutConflicts[keyStroke.vim] = ShortcutOwnerInfo.allUndefined
        }
  ...
  private object LookupKeys {
    fun isEnabledForLookup(keyStroke: KeyStroke): Boolean {
      ...
      return keyStroke.vim !in parsedLookupKeys
    }
