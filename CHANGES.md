...
* [VIM-4308](https://youtrack.jetbrains.com/issue/VIM-4308) Fixed the [`CurSearch`](https://vimhelp.org/syntax.txt.html#hl-CurSearch) highlight staying on the screen after the search was over. As in Vim, the match the cursor is on is drawn with a box around it only while [`'incsearch'`](https://vimhelp.org/options.txt.html#%27incsearch%27) is previewing the pattern you are typing; accepting the search with `<CR>` now removes the box, and `n`, `N` and ordinary caret movement no longer carry it from match to match, so once the search is finished every [`'hlsearch'`](https://vimhelp.org/options.txt.html#%27hlsearch%27) match looks the same again
