...
* Fixed the caret position after [`r`](https://vimhelp.org/change.txt.html#v_r) in Visual mode — replacing a selection left the caret at the end of the replaced text instead of on its first character. With the caret on the first `f` of `abcdffffff`, `v2lra` now leaves it on the first of the three `a`s, as in Vim. The same applies to Visual line and Visual block replaces, and to repeating one with [`.`](https://vimhelp.org/repeat.txt.html#single-repeat)
