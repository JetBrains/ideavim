...
data class WritePreviousFileCommand(val range: Range, val argument: String) : Command.SingleExecution(range, argument) {
  ...
  override fun processCommand(editor: VimEditor, context: ExecutionContext, operatorArguments: OperatorArguments): ExecutionResult {
    // Note that `:[count]wp[revious] [++opt] {file}` does not get count from argument. Vim would parse a count argument
    // to be the file name
