...
interface VimMarkService {
  companion object {
    const val SELECTION_START_MARK = '<'
    const val SELECTION_END_MARK = '>'

    const val CHANGE_START_MARK = '['
    const val CHANGE_END_MARK = ']'

    const val PARAGRAPH_START_MARK = '{'
    const val PARAGRAPH_END_MARK = '}'

    const val SENTENCE_START_MARK = '('
    const val SENTENCE_END_MARK = ')'

    const val BEFORE_JUMP_MARK = '\''
    const val INSERT_EXIT_MARK = '^'
    const val LAST_BUFFER_POSITION = '"'
    const val LAST_CHANGE_MARK = '.'

    const val UPPERCASE_MARKS = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    const val LOWERCASE_MARKS = "abcdefghijklmnopqrstuvwxyz"
    const val NUMBERED_MARKS = "0123456789"
  }
