...
fun VimKeyGroup.getAllMappingInfoWithMode(prefix: List<KeyStroke>, modes: Set<MappingMode>): List<MappingInfoWithMode> {
  ...
  modes.forEach { mode ->
    ...
    // Vim includes mappings for each key in the prefix, where appropriate. That is, it doesn't just use list mappings
    // that are descendants of the prefix but includes the mappings for each key in the prefix as well.
    ...
    while (iterator.hasNext()) {
      ...
      if (originalModes.size == 1) {
        ...
        // This map was originally registered with multiple modes, so will likely have multiple entries. Use the first
        // one, remember that it's been used and add it to the results with the current possibly changed set of modes.
