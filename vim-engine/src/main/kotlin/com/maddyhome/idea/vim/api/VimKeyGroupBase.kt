...
abstract class VimKeyGroupBase : VimKeyGroup {
  ...
  private fun registerKeyMapping(fromKeys: List<VimKeyStroke>, owner: MappingOwner) {
    ...
    for (key in fromKeys) {
      if (key !is VimKeyStroke.Special && key !is VimKeyStroke.Combination) continue
      if (!injector.application.isOctopusEnabled() || key != VimKeyStroke.CR && key != VimKeyStroke.ESC) {
