...
abstract class VimSearchGroupBase : VimSearchGroup {
  ...
  fun parseGlobalCommand(argument: String): GlobalCommandArguments? {
     ...
     */
    if (argument.isEmpty()) {
      injector.editorGroup.getFocusedEditor()?.let { messages.showErrorMessage(it, messages.message("E148")) }
      messages.indicateError()
      return null
    } else if (cmd.charAt() == '\\') {
      cmd.inc()
      if ("/?&".indexOf(cmd.charAt()) == -1) {
        injector.editorGroup.getFocusedEditor()?.let { messages.showErrorMessage(it, messages.message("E10")) }
        return null
      }
  ...
  override fun processSearchCommand(
    ...
    if (pattern.isNullOrEmpty()) {
      ...
      if (pattern.isNullOrEmpty()) {
        ...
        pattern = lastSubstitutePattern
        if (pattern.isNullOrEmpty()) {
          injector.messages.showErrorMessage(editor, "E35: No previous regular expression")
          return null
        }
      }
    ...
    if (!doAsk) {
      ...
      if (lineToNextSubstitute == null) {
        ...
        injector.editorGroup.getFocusedEditor()
          ?.let { injector.messages.showErrorMessage(it, "E486: Pattern not found: $pattern") }
        return true
      }
  ...
  private fun postSubstitute(
    ...
    if (exceptions.isNotEmpty()) {
      injector.messages.indicateError()
      injector.editorGroup.getFocusedEditor()?.let { injector.messages.showErrorMessage(it, exceptions[0].message) }
    }
  }
  ...
  private fun parseSubstituteCommand(
    ...
    if (excmd[0] == 's' && exarg.isNotEmpty() && !exarg.first()
      ...
      if (exarg.first().isLetter()) {
        injector.editorGroup.getFocusedEditor()
          ?.let { injector.messages.showErrorMessage(it, "E146: Regular expressions can't be delimited by letters") }
        return null
      }
      ...
      if (exarg.first() == '\\') {
        if (exarg.length < 2 || !"/?&".contains(exarg[1])) {
          injector.editorGroup.getFocusedEditor()
            ?.let { injector.messages.showErrorMessage(it, "E10: \\ should be followed by /, ? or &") }
          return null
        }
        if (exarg[1] != '&') {
      ...
      if (lastSubstituteString == null) {
        ...
        injector.editorGroup.getFocusedEditor()
          ?.let { injector.messages.showErrorMessage(it, "E33: No previous substitute regular expression") }
        return null
      }
      pattern = null
    ...
    if (trailingOptionsEndIndex < exarg.length && exarg[trailingOptionsEndIndex].isDigit()) {
      ...
      if (count <= 0 && doError) {
        injector.editorGroup.getFocusedEditor()
          ?.let { injector.messages.showErrorMessage(it, injector.messages.message("command.substitute.zero.count")) }
        return null
      }
      line1 = line2
    ...
    if (trailingOptionsEndIndex < exarg.length && exarg[trailingOptionsEndIndex] != '"') {
      ...
      injector.editorGroup.getFocusedEditor()?.let {
    ...
    if (pattern == null) {
      ...
      // Pattern was never defined
      if (pattern == null) {
        injector.editorGroup.getFocusedEditor()?.let { injector.messages.showErrorMessage(it, errorMessage) }
        return null
      }
    }
