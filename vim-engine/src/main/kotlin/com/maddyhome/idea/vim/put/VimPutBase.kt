...
abstract class VimPutBase : VimPut {
  ...
  override fun putText(
    ...
    deleteSelectedText(editor, context, data, operatorArguments, saveToRegister)
  ...
  @RWLockLabel.SelfSynchronized
  private fun deleteSelectedText(editor: VimEditor, context: ExecutionContext, caret: VimCaret, data: PutData, operatorArguments: OperatorArguments, saveToRegister: Boolean): VimCaret? {
    ...
    injector.application.runWriteAction {
      injector.changeGroup.deleteRange(editor, context, caret, range, selectionForCaret.type, false, operatorArguments, saveToRegister)
    }
    return caret.moveToInlayAwareOffset(range.startOffset)
  }

  @RWLockLabel.SelfSynchronized
  private fun deleteSelectedText(editor: VimEditor, context: ExecutionContext, data: PutData, operatorArguments: OperatorArguments, saveToRegister: Boolean) {
    ...
    data.visualSelection.caretsAndSelections.entries.sortedByDescending { it.key.getBufferPosition() }
      .forEach { (caret, selection) ->
        ...
        injector.application.runWriteAction {
          injector.changeGroup.deleteRange(editor, context, caret, range, selection.type, false, operatorArguments, saveToRegister)
        }
