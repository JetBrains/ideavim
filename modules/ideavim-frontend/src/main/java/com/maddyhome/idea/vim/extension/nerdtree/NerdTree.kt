...
private fun createMappings(): Map<List<KeyStroke>, NerdTreeAction> = navigationMappings.toMutableMap().apply {
  ...
  //   monolith:    injector.file → FileGroup.openFile() → FileEditorManager.openFile()
  //   split mode:  injector.file → FileGroupSplitClient.openFile() → RPC → FileRemoteApiImpl → FileGroup
  register(
